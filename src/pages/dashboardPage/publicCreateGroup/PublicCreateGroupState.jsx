...
const PublicCreateGroupState = ({ groupName, setStep, addUserList }) => {
  ...
  const createGroup = async () => {
    ...
    const group_creater = session.user.id;
  ...
  return (
    <div className="bg-gray-900 p-6 rounded-2xl shadow-lg">
      ...
      <div className="mb-4 flex items-center gap-3">
        ...
        <h2 className="text-2xl text-white font-bold">{groupName}</h2>
      </div>
      <div>
      ...
      {addUserList.length > 0 && (
        <div className=" mt-5  flex justify-end items-center">
          ...
          >
            {loading ? <span>Creating ...</span> : <span>Create Group</span>}
          </button>
        </div>
      )}
    </div>
...
export default PublicCreateGroupState;
